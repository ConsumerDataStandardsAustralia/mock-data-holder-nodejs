import { BankingAccountV2, BankingAccountDetailV3, BankingProductV4, BankingTransaction, BankingBalance, BankingDirectDebit, BankingScheduledPaymentFrom, BankingScheduledPaymentV2, BankingPayeeV2, BankingPayeeDetailV2 } from "consumer-data-standards/banking";
import { CustomerModel } from "../models/login";

export interface IBankingData {

    getBankingProductDetails(productId: string): Promise<BankingProductV4>;

    getAllBankingProducts(queryParameters: any): Promise<BankingProductV4[]>;

    getAccounts(customerId: string, queryParameters: any): Promise<BankingAccountV2[]>;

    getAccountDetail(customerId: string, accountId: string): Promise<BankingAccountDetailV3>;

    getTransationsForAccount(customerId: string, accountId: string, queryParameters: any): Promise<BankingTransaction[]>;

    getTransactionDetail(customerId: string, accountId: string, transactionId: string): Promise<BankingTransaction>;

    getBulkBalances(customerId: string, queryParameters: any): Promise<BankingBalance[]>;

    getAccountBalance(customerId: string, accountId: string): Promise<BankingBalance>;
    ...
    getPayeeDetail(customerId: string, payeeId: string): Promise<BankingPayeeDetailV2>;
